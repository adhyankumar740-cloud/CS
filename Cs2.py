...
def room_booking(user_details):
    ...
    k = user_details[0][0] # user's id
    y = user_details[0][1] # user's name
...
def insert_sdetails(user_details):
    ...
    p = user_details[0][0] # user id (from current session)
    k = user_details[0][1] # user name (from current session)
...
def insert_gdetails(user_details):
    ...
    mydb = m.connect(host="localhost", user="root", password="gouravbhai@12", database="taj_hotel")
    co = mydb.cursor()
    p = user_details[0][0] # user id
    k = user_details[0][1] # user name
    h = int(input("Enter phone number:"))
    l = input("Enter address:")
    s_gender = input("Enter gender:")
    w = "GUEST"
...
if __name__ == "__main__":
    ...
    s = [] # User details variable, no longer global in scope, but locally maintained
